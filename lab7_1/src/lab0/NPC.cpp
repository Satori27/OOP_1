...
NPC::NPC(NpcType t, int _x, int _y, int _dist) : type(t), x(_x), y(_y), dist(_dist) {}
NPC::NPC(NpcType t, std::istream &is, int dist) : type(t)
{
    is >> x;
    is >> y;
    is >> dist;
...
bool NPC::is_close(const std::shared_ptr<NPC> &other)
{
    ...
    if ((std::pow(x - other_x, 2) + std::pow(y - other_y, 2)) <= std::pow(dist, 2))
        return true;
    else
        return false;
}
